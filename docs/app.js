...
(function () {
  ...
  function buildField(field) {
    switch (field.type) {
      ...
      case 'date': return buildDateTimeField(field, 'date');
      case 'time': return buildDateTimeField(field, 'time');
  ...
  /** Native date/time pickers — these give a fixed, unambiguous format
   * (the browser handles locale display) instead of free-typed dates like
   * "8/4/26" vs "04-08-2026" that are easy to misread or miskey. An
   * explicit "Not applicable" checkbox stands in for the usual N/A text,
   * since a date/time input can't hold the literal string "N/A". */
  function buildDateTimeField(field, kind) {
    var wrap = el('div', 'field');
    wrap.setAttribute('data-field-id', field.field_id);
    wrap.appendChild(el('label', 'field-label', field.label));
